...
$$x_{mont} = x \cdot R \mod p$$

Where $R = 2^k$ for some $k \geq \log_2(p)$. Montgomery multiplication avoids expensive division operations.
...
$\mathbb{F}_{p^2}$ is a degree-2 extension of $\mathbb{F}_p$, analogous to complex numbers:
...
$\mathbb{F}_{p^6}$ is built as a cubic extension over $\mathbb{F}_{p^2}$:

$$\mathbb{F}_{p^6} = \mathbb{F}_{p^2}[v] / (v^3 - \xi)$$

Where $\xi = u$ (the imaginary unit from $\mathbb{F}_{p^2}$), so $v^3 = u$.

Elements have the form $c_0 + c_1 \cdot v + c_2 \cdot v^2$ where $c_0, c_1, c_2 \in \mathbb{F}_{p^2}$.
...
**BLS6_6 G1 curve:** $y^2 = x^3 + 6$ over $\mathbb{F}_{43}$ ($a = 0$, $b = 6$)
...
Given a point $P$ and scalar $k$, compute $k \cdot P = \underbrace{P + P + \cdots + P}_{k \text{ times}}$.

### Double-and-Add Algorithm

The standard binary method (currently implemented):

**Input:** $P$, $k = (k_{n-1}, \ldots, k_1, k_0)_2$  
**Output:** $k \cdot P$
...
&\quad \textbf{if } k_i = 1: R \leftarrow R + P \\
&\quad P \leftarrow 2P \\
&\textbf{return } R
\end{aligned}
$$
...
$$
\begin{aligned}
&R_0 \leftarrow \mathcal{O}, \quad R_1 \leftarrow P \\
&\textbf{for } i \text{ from } n-1 \text{ down to } 0: \\
&\quad \textbf{if } k_i = 0: R_1 \leftarrow R_0 + R_1, \quad R_0 \leftarrow 2 \cdot R_0 \\
&\quad \textbf{else: } R_0 \leftarrow R_0 + R_1, \quad R_1 \leftarrow 2 \cdot R_1 \\
&\textbf{return } R_0
\end{aligned}
$$
...
**BLS6_6 in lumen-curve:** $k = 6$, using a sextic twist
...
**Output:** $f_{r,P}(Q)$ before final exponentiation

$$
\begin{aligned}
&f \leftarrow 1, \quad T \leftarrow P \\
&\textbf{for } i \text{ from } n-2 \text{ down to } 0: \\
&\quad f \leftarrow f^2 \cdot \ell_{T,T}(Q) \quad \text{// line through } T,T \text{ evaluated at } Q \\
&\quad T \leftarrow 2T \\
&\quad \textbf{if } r_i = 1: f \leftarrow f \cdot \ell_{T,P}(Q), \quad T \leftarrow T + P \\
&\textbf{return } f
\end{aligned}
$$
...
For polynomial $P(x) = \sum_i c_i x^i$:

$$C = \sum_i c_i \cdot [\tau^i]G = [P(\tau)]G$$
...
4. $r = x_1 \mod n$ (if $r = 0$, go to step 2)
...
2. $v_1 = bd$
3. $v_2 = (a+b)(c+d)$
4. Result: $(v_0 - v_1) + (v_2 - v_0 - v_1)u$
